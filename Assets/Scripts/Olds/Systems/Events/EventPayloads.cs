...
public readonly struct CastStarted
{
	...
	public readonly ISkillParams Params;    // 시전 시점의 파라미터 스냅샷(읽기 전용 계약)
	public readonly Transform Caster;     // 편의상 별도 보강
	public readonly Transform? TargetOpt; // 대상형 스킬일 때만 존재(없으면 null)

	public CastStarted(GameEventMeta meta, SkillRef skill, ISkillParams @params, Transform caster, Transform? targetOpt)
	{ Meta = meta; Skill = skill; Params = @params; Caster = caster; TargetOpt = targetOpt; }
...
/// - ArmorPenetration: 총 방어 관통 계수(곱연산, <see cref="StatsBridge.TotalArmorPenetration()"/> 참조)
